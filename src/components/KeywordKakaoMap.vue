  ...
  watch: {
    chargers() {
      ...
      this.chargers.forEach((charger) => {
        ...
        obj.latlng = new kakao.maps.LatLng(charger.latitude, charger.longitude);
    ...
    loadMaker() {
      ...
      this.positions.forEach((position) => {
        ...
        let content =
          ...
          "	 <div>" +
          '    <img class="overlayImg" src=' +
          position.img +
          ' style="height:200px; width:100%"/>' +
          '    </div><div class="accommInfoWrap">' +
          "<div>" +
          '    <p3 class="accommName" style="font-size:18px;font-weight: 600;text-align:center;color:#00B98E;">' +
          position.title +
          "</p3>" +
          '      </div><div ><p class="accommRegion" style="font-size:12px;">' +
          position.add +
          "</p>" +
          "    </div></div>" +
        ...
        kakao.maps.event.addListener(
          marker,
          "mouseover",
          this.makeOverListener(this.map, marker, addwindow)
        );
        kakao.maps.event.addListener(
          marker,
          "click",
          this.makeclickListener(this.map, marker, position)
        );
